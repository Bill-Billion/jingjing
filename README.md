...
最新讨论使用“**晶选片场**”作为产品名称，围绕“建立数字人 → 入戏 → 培育 → 成角”重新组织业务。现有工程仍保留“晶晶日上”的名称与旧业务流程；新版目标、现有实现和待决策事项可在[项目全景](项目全景/index.html)中对照查看。

> **当前状态：可用于开发、演示和继续联调，尚需完成真实交易链路与生产验收。** 页面流程、内置演示数据和第三方服务代码同时存在；看到“支付成功”或生成结果，并不代表对应能力已完成真实渠道验收。工程运行验证记录来自 **2026-09-10**，详见[项目审查记录](docs/project-review-2026-09-10.md)；**2026-09-18** 新增了基于本地源码与最新讨论的项目全景分析，本次未进行线上或真实渠道验收。

[项目全景与最新讨论](#项目全景与最新讨论) · [功能与流程](#功能与流程) · [技术架构](#技术架构) · [目录导航](#目录导航) · [快速开始](#快速开始) · [第三方服务配置](#第三方服务配置) · [测试与验证](#测试与验证) · [已知限制与后续工作](#已知限制与后续工作) · [文档索引](#文档索引)

## 项目全景与最新讨论

**[项目全景网页入口](项目全景/index.html)** 将项目设计、需求、新讨论及其与旧版 App 的差距汇总为五个可交互视图。建议先读全景，再根据页面标出的文档章节和代码位置核对细节。

| 视图 | 内容与交互 |
| --- | --- |
| 业务全景 | 点击四个业务阶段，查看现状、目标、分期及依据；区分私人自娱与公开发行两条业务线 |
| 流程与界面 | 切换定制剧、数字人创建、授权与审核三组新旧流程；查看旧 App 实际截图和已有视觉方案 |
| 新旧差距 | 14 项差距，可按重构、贯通、补齐、重组、后置筛选，并展开证据 |
| 需求与分期 | 查看 P0 付费交付、P1 经营变现、P2 公开发行的范围及 P0 依赖顺序 |
| 讨论与待决策 | 按 A / B / C / D 分组浏览 32 项问题，展开选项、推荐、影响范围及工作底稿行号 |

### 打开方式

下载或克隆仓库后，直接用浏览器打开 `项目全景/index.html`。网页使用原生 HTML、CSS 和 JavaScript，数据及图片均随仓库提供，无需安装依赖、构建或启动业务后端。
...
然后访问 [本地项目全景](http://127.0.0.1:8768/项目全景/)。Windows 可将 `python3` 换成 `py`。GitHub 文件页面显示 HTML 源码，交互效果需要本地浏览器打开。

### 最新讨论改变了什么

| 主题 | 现有 App / 工程 | 最新讨论中的目标 |
| --- | --- | --- |
| 定制剧交易 | 普通档以 99 元意向金、剧本确认、4901 元制作款、成片验收推进 | 按形象、主题、框架、陪演等配置报价；首款后制作样片，样片满意后付尾款，再交付成片 |
| 数字人资产 | 创建艺人档案、选择授权范围，并提供部分接单能力 | 区分纯虚拟、本人、他人数字人；把权利人、授权证据与有效状态接到陪演、接单、发行等动作 |
| 作品与审核 | 已有作品、项目、机审与人审等分散能力 | 私人收藏和公开发行分线，发行需补齐授权并进入更严格的审核流程 |
| 交付顺序 | 祝福、代言、定制剧、角色席位等入口并列 | P0 先贯通私人定制付费交付与授权审核基础；P1 扩展经营变现，P2 实现竞拍及正式发行 |

本次全景是分析与评审材料，未修改 App 或后端业务逻辑。已有视觉方案展示的是界面风格，不能据此认定新版业务已经落地。

### 讨论来源与判断口径

| 资料 | 定位 |
| --- | --- |
| [最新会议.docx](最新会议.docx) | 《晶选片场（数字人 APP）产品业务逻辑与前后端落地方案》V0.4 评审稿，包含新版业务、流程与授权设计 |
| [业务架构融合与待处理问题清单](晶选片场_业务架构融合与待处理问题清单_20260915.md) | 2026-09-15 融合整理，衔接旧问题、补充实现缺口与分期 |
| [业务决策工作底稿](晶选片场_业务决策工作底稿_20260915.xlsx) | 32 项待决策问题及数字人开放矩阵，用于记录讨论与最终结论 |

截至本次核对，工作底稿的 32 项会议结论均未填写。**首款 50%、平台抽佣 20%、免费修改 2 轮等是推荐值，尚未决议。** 全景分别标注现有实现、目标方向和待定建议；历史产品价格档案用于解释旧版，新版规则需以讨论后补齐的明确决议为准。
...
以上描述现有工程。旧业务档案将角色席位定义为内容消费与交付权益；旧版价格、角色权益和状态机见[产品与价格单一事实源](晶晶日上工程交接包/02_项目档案/01_产品与价格单一事实源.md)。新版变更还需对照[最新讨论与待决策事项](#讨论来源与判断口径)，避免把旧版定价或新稿推荐直接当作最终规则。
...
.
├─ README.md                              # 项目入口
├─ 项目全景/                             # 设计、需求、讨论与新旧差距的交互网页
│  ├─ index.html                         # 浏览器入口
│  ├─ data.js / app.js / style.css        # 分析数据、交互与样式
│  └─ assets/                            # 旧 App 截图及已有视觉方案截图
├─ 最新会议.docx                         # 晶选片场 V0.4 评审稿
├─ 晶选片场_业务架构融合与待处理问题清单_20260915.md
├─ 晶选片场_业务决策工作底稿_20260915.xlsx
├─ docs/
...
| 阅读目的 | 文档 |
| --- | --- |
| 项目设计、需求与新旧差距 | [项目全景交互网页](项目全景/index.html)（[打开说明](#打开方式)） |
| 最新业务与流程讨论 | [V0.4 评审稿](最新会议.docx)、[融合与待处理问题清单](晶选片场_业务架构融合与待处理问题清单_20260915.md) |
| 待决策事项与会议结论 | [业务决策工作底稿](晶选片场_业务决策工作底稿_20260915.xlsx) |
...
| 旧版产品、价格和权益 | [产品与价格单一事实源](晶晶日上工程交接包/02_项目档案/01_产品与价格单一事实源.md) |
