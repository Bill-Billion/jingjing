...
async function main(env=process.env) {
 ...
 try {
  ...
  const app=createAccountApi({db,secret:env.AUTH_SECRET_BASE64,sms:createSmsProvider(db,env),authSettings:settings,allowedOrigins:(env.API_ALLOWED_ORIGINS||'').split(',').filter(Boolean),governanceEnvironment:env.NODE_ENV==='production'?'PRODUCTION':'SANDBOX',supplyEnv:env});
