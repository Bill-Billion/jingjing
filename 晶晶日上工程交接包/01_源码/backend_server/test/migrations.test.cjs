...
test('场景1 全新库：一次迁移到最新，版本齐全、新增工程表存在', () => {
  ...
  try {
    ...
    assert.deepStrictEqual(migrated, [1, 2, 3, 4, 5, 6]);
    assert.deepStrictEqual(appliedVersions(db).map(Number), [1, 2, 3, 4, 5, 6]);
...
test('场景3 重复执行幂等：第二次全部跳过、结构与版本不重复', () => {
  ...
  try {
    ...
    assert.strictEqual(db.prepare('SELECT COUNT(*) c FROM schema_migrations').get().c, 6);
    // 再来一次仍稳定
...
test('006 adds sample URL and classifies no historical digest; existing manually added URL survives', () => {
  for (const existingUrl of [false, true]) {
    const db = new Database(':memory:');
    try {
      require(path.join(MIG_DIR, '001_initial.js')).up(db);
      if (existingUrl) db.exec('ALTER TABLE sample_library ADD COLUMN video_url TEXT');
      db.prepare("INSERT INTO sample_library(id,title,genre,outline) VALUES (1,'existing sample','test','synthetic outline')").run();
      db.prepare("INSERT INTO scripts(id,scriptwriter_id,title,evidence_hash) VALUES (1,1,'existing script','original digest')").run();
      if (existingUrl) db.prepare("UPDATE sample_library SET video_url='https://example.invalid/old' WHERE id=1").run();
      runMigrations(db, { logger: quietLogger() });
      runMigrations(db, { logger: quietLogger() });
      const sample = db.prepare('SELECT * FROM sample_library WHERE id=1').get();
      assert.strictEqual(sample.title, 'existing sample');
      assert.strictEqual(sample.video_url, existingUrl ? 'https://example.invalid/old' : null);
      const script = db.prepare('SELECT * FROM scripts WHERE id=1').get();
      assert.strictEqual(script.evidence_hash, 'original digest');
      assert.strictEqual(script.digest_kind, null);
      assert.strictEqual(cols(db, 'sample_library').filter(c => c === 'video_url').length, 1);
    } finally { db.close(); }
  }
});
