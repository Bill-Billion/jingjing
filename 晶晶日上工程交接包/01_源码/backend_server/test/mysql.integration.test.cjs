...
test('Real isolated MySQL8 foundation', {skip:!process.env.JX_MYSQL_TEST_ENV_FILE}, async(t)=>{
  ...
  try {
    ...
    await t.test('status on a fresh database is read-only',async()=>{
      const state=await status(db);assert.deepEqual(state.migrations.map(x=>x.status),['PENDING','PENDING']);assert.equal(state.history.length,0);
    ...
    await t.test('new schema applies once, repeated migration is a no-op',async()=>{
      assert.deepEqual((await migrate(db)).applied,['0001','0002']);assert.deepEqual((await migrate(db)).applied,[]);
      const state=await status(db);assert.equal(state.history.length,2);assert.ok(state.history.every(x=>x.status==='APPLIED'));
