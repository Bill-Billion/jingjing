...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    // Gate0 隔离实验台：变体切换（banner/艺人卡元素开关）后重建本页
  ...
  Widget _buildBanner() {
    // Gate0 隔离实验台 V2：整块隐藏 Banner，测其照片+阴影+文字层总成本
    if (PerfIso.noBanner) return const SizedBox.shrink();
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          // Gate0 隔离实验台 V5：移除两层模糊阴影
          boxShadow: PerfIso.noBannerShadows
              ? null
              : [
                  BoxShadow(
        ...
        child: ClipRRect(
          ...
          child: Stack(
            fit: StackFit.expand,
            children: [
              // Gate0 隔离实验台 V6：照片替换为渐变占位
              if (PerfIso.noBannerPhoto)
                const DecoratedBox(
                  decoration: BoxDecoration(
    ...
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          // Gate0 隔离实验台 V3：移除两层模糊阴影（保留封面/渐变/文字）
          boxShadow: PerfIso.noArtistShadows
              ? null
              : [
                  BoxShadow(
        ...
        child: ClipRRect(
          ...
          child: Stack(
            fit: StackFit.expand,
            children: [
              // Gate0 隔离实验台 V4：封面照片替换为品牌渐变占位，隔离大图合成成本
              if (PerfIso.noArtistPhotos)
              ...
              else if (a['remoteImg'] != null && '${a['remoteImg']}'.isNotEmpty)
                Image.network(
                ...
                _artistCoverPlaceholder(),
              // Gate0 隔离实验台 V7：移除底部「透明→黑」渐变遮罩
              if (!PerfIso.noArtistGradients)
                Positioned.fill(
                  child: DecoratedBox(
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
