// A8：锁定定制剧样片订单详情错误态——online 远端 getSampleOrder 失败时落统一
// ErrorView（"加载失败"+"订单加载失败"+"重新加载"），重试可再次发起、失败后稳定不崩。
...
void main() {
  ...
  testWidgets('online 远端失败：样片订单详情落 ErrorView，可重试不崩', (t) async {
    await t.pumpWidget(MaterialApp(
      debugShowCheckedModeBanner: false,
      theme: AppTheme.darkTheme,
      home: const SampleOrderDetailPage(orderId: 1),
    ));
    await settle(t);

    expect(find.text('加载失败'), findsOneWidget);
    expect(find.text('订单加载失败'), findsOneWidget);
