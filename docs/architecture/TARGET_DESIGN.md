...
|数据|MySQL8/InnoDB，正式优先托管；SQLite仅旧库来源/隔离测试；媒体入私有OSS|旧运行时SQLite；MySQL脚本不等于已支持MySQL|
|后端|Node/Express JavaScript模块化单体，async repository/service、mysql2/promise；独立Worker|当前未完成上述改造|
|队列|MySQL jobs/lease/outbox；API与Worker进程分开；不用Redis/BullMQ/Kafka/ES/微服务|原scheduler不能视为持久任务实现|
|契约|REST + OpenAPI3.1；金额minor/currency、request_id、idempotency_key、object_version、actor/acting_party、current_status/allowed_actions|`contracts/openapi.yaml`已形成0.1.0-rc.1首批候选，待Experience交叉review，未合入integration；新端点尚未实现|
