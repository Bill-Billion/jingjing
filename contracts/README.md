...
当前候选版本0.4.0-rc.1，主文件[openapi.yaml](openapi.yaml)。共40个操作，其中35个已接入独立MySQL账号API并进行隔离测试；其余5个仍未实现。具体状态见各操作的x-implementation-status及[实现清单](implementation.json)。这些是后端自验结果，不是短信供应商已开通、前端已联调或正式上线。
